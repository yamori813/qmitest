   ...
   u16      bufferLen )
{
   ...
   for (pos = 4; 
        ...
        pos += tlvSize + 3)
   {
      tlvSize = *(u16 *)((u8 *)pQMIMessage + pos + 1);
      if (*(u8 *)((u8 *)pQMIMessage + pos) == type)
      {
         ...
         {
            *((char*)((u8 *)pOutDataBuf + cpyCount)) = *((char*)((u8 *)pQMIMessage + pos + 3 + cpyCount));
         }
   ...
   u8       serviceType )
{
   ...
   *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ))= 0x00;
   // Transaction ID
   *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 1) = transactionID;
   // Message ID
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 2) = 0x0022;
   // Size of TLV's
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 4) = 0x0004;
      // QMI Service Type
      *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 6)  = 0x01;
      // Size
      *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 7) = 0x0001;
      // QMI svc type
      *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 9)  = serviceType;
   ...
   u16      clientID )
{
   ...
   *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 1 ) = transactionID;
   // Message ID
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 2) = 0x0023;
   // Size of TLV's
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 4) = 0x0005;
      // Release client ID
      *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 6)  = 0x01;
      // Size
      *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 7) = 0x0002;
      // QMI svs type / Client ID
      *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 9)  = clientID;
   ...
   u8       transactionID )
{
   ...
   *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 1) = transactionID;
   // Message ID
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 2) = 0x0021;
   // Size of TLV's
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 4) = 0x0000;
   ...
   u16      transactionID )
{
   ...
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 1) = transactionID;
   // Message ID
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 3) = 0x0001;
   // Size of TLV's
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 5) = 0x0008;
      // Report channel rate TLV
      *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 7)  = 0x11;
      // Size
      *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 8) = 0x0005;
      // Stats period
      *(u8 *)((u8 *)pBuffer + sizeof( sQMUX ) + 10)  = 0x01;
      // Stats mask
      *(u32 *)((u8 *)pBuffer + sizeof( sQMUX ) + 11)  = 0x000000ff;
   ...
   u16      transactionID )
{
   ...
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 1) = transactionID;
   // Message ID
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 3) = 0x0022;
   // Size of TLV's
   ...
   u16      transactionID )
{
   ...
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 1) = transactionID;
   // Message ID
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 3) = 0x0025;
   // Size of TLV's
   *(u16 *)((u8 *)pBuffer + sizeof( sQMUX ) + 5) = 0x0000;
   ...
   u16 *  pClientID )
{
   ...
   result = GetQMIMessageID( (u8 *)pBuffer, buffSize );
   if (result != 0x22)
   {
      return -EFAULT;
   }

   result = ValidQMIMessage( (u8 *)pBuffer, buffSize );
   if (result != 0)
   {
      return -EFAULT;
   }
   ...
   int      meidSize )
{
   ...
   result = GetTLV( pBuffer, buffSize, 0x11, (void*)pMEID, meidSize );
   if (result <= 0)
   {
      return -EFAULT;
   }
